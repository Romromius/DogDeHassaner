...
def avi_to_frames(video_path, target_size=(256, 256)):
    ...
    manifest_reader.spawn_json("manifest.json")

    reader = manifest_reader.ManifestReader("manifest.json")
    ...
    while True:
        ...
        output_path = os.path.join(output_dir, f"{frame_count + 1}.png")
        cv2.imwrite(output_path, gray_frame)

        frame_count += 1

        reader.add_frame(frame_count)

    reader.set_total(frame_count)
    reader.commit()
